import { GET_USER_DELIVERY_ADDRESSES, SET_USER_DELIVERY_ADDRESS_STREET1, SET_USER_DELIVERY_ADDRESS_STREET2, SET_DELIVERY_ADDRESS_CITY } from '../constants/actionTypes.ts';
...
export default function user_delivery_addresses(state: any = [], action: any){
  ...
  switch (action.type) {
    ...
    case SET_USER_DELIVERY_ADDRESS_STREET1:
      ...
      delivery_address_updated = state.addresses.filter((address: any) => {

            //if(address.address)
            console.log("id" + address.address_id);
            if(address.address_id === 1){
                
                address.street_address1 = "home2";
    ...
    case SET_USER_DELIVERY_ADDRESS_STREET2:
